...
public class Game {
    ...
    public String checkGameWinner(char[][] grid) {
        String result = "None";
        //Boxes refer to the grid position i.e. boxOne equals first cell, boxTwo equals second horizontal cell
        char boxOne = grid[0][0];
        char boxTwo = grid[1][0];
        char boxThree = grid[2][0];
        char boxFour = grid[0][1];
        char boxFive = grid[1][1];
        char boxSix = grid[2][1];
        char boxSeven = grid[0][2];
        char boxEight = grid[1][2];
        char boxNine = grid[2][2];
        //Student code goes here ...
        if (freeSpots >= 0) {
            if (boxOne != '-' && ((boxOne == boxTwo && boxOne == boxThree) ||
                    (boxOne == boxFour && boxOne == boxSeven) ||
                    (boxOne == boxFive && boxOne == boxNine))) {
                result = boxOne + " wins";
            } else if (boxTwo != '-' && boxTwo == boxFive && boxTwo == boxEight) {
                result = boxTwo + " wins";
            } else if (boxThree != '-' && ((boxThree == boxSix && boxThree == boxNine) ||
                    (boxThree == boxFive && boxThree == boxSeven))) {
                result = boxThree + " wins";
            } else if (boxFour != '-' && boxFour == boxFive && boxFour == boxSix) {
                result = boxFour + " wins";
            } else if (boxSeven != '-' && boxSeven == boxEight && boxSeven == boxNine) {
                result = boxSeven + " wins";
            } else if(freeSpots == 0){
                result = "Tie";
            }
        }
